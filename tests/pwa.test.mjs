...
assert.match(html, /<script src="src\/pwa-register\.js\?v=profile-edit-3"><\/script>/, "index should load the external PWA registration script");
...
assert.match(serviceWorker, /const cacheName = "shanhai-yingji-v6"/, "service worker should version its cache");
assert.match(serviceWorker, /"\.\/assets\/avatar-lin-che\.svg"/, "service worker should cache the profile avatar");
assert.match(serviceWorker, /"\.\/src\/main\.js\?v=profile-edit-3"/, "service worker should cache the app entry");
assert.match(serviceWorker, /"\.\/src\/storage\/archive-export\.js\?v=profile-edit-3"/, "service worker should cache archive transfer helpers");
assert.match(serviceWorker, /"\.\/src\/pwa-register\.js\?v=profile-edit-3"/, "service worker should cache PWA registration");
